...
#if defined(NDEBUG)
# error "neptunecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6a904340995a20ffb57d5711bc3ab02654a330068eefb0b6961005d9cbcc041e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // neptunecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "neptunecoin Signed Message:\n";
...
{
    // neptunecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 180 * COIN;

            if(nHeight == 1)
            {
            nSubsidy = 705600000 * COIN;
            }
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // neptunecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // neptunecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1472989507, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "The Times 03Jan2009 Chancellor on brink of second bailout for banks";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 180 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040237d9b4e5e686a4e165267bf4a0bb5833bf77b3aabae44ba119991776138eedb73035bbda4155530acd1256e5597d9da0efb8b44e7dab8cca966e3e8be6dbf2") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1472989507;
        ...
        assert(block.hashMerkleRoot == uint256("0x49743e0582a331e71ba9c898cda04471df2a974dbcda9753f7ed8e004e2b570c"));
...
unsigned char pchMessageStart[4] = { 0xbd, 0x0a, 0xe9, 0x82 }; // neptunecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("neptunecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("neptunecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static neptunecoinMiner(CWallet *pwallet)
{
    printf("neptunecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("neptunecoin-miner");
    ...
    try { loop {
        ...
        printf("Running neptunecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("neptunecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&neptunecoinMiner, pwallet));
}
